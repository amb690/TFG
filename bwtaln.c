...
void bwa_cal_sa_reg_gap(int IGP_id_thread, int tid, bwt_t *const bwt, int n_seqs, bwa_seq_t *seqs, const gap_opt_t *opt)
{
	IGP_Begin_Thread(IGP_id_thread, n_seqs);
	...
	for (i = 0; i != n_seqs; ++i) {
		...
		IGP_Report(IGP_id_thread, 1);
	}
	free(seed_w); free(w);
	gap_destroy_stack(stack);

	IGP_End_Thread(IGP_id_thread);
}
...
typedef struct {
	...
	bwa_seq_t *seqs;
	const gap_opt_t *opt;
int IGP_id_thread; // Identificador de la hebra para IGP
} thread_aux_t;

static void *worker(void *data)
{
	thread_aux_t *d = (thread_aux_t*)data;
	bwa_cal_sa_reg_gap(d->IGP_id_thread, d->tid, d->bwt, d->n_seqs, d->seqs, d->opt);
	return 0;
}
#endif
...
void *consumoTotal(void *ptr)
{
    unsigned long type = (unsigned long) ptr;
    type = Read_Power();
    fprintf(stderr, "Initial Power[%lu]\n", type);
    while (Stop==0) {
	type+=Read_Power();
	sleep(1);
    }
    fprintf(stderr, "Total Power[%lu]\n", type);
...
#ifdef HAVE_PTHREAD
		if (opt->n_threads <= 0) { // no multi-threading at all
			bwa_cal_sa_reg_gap(0, 0, bwt, n_seqs, seqs, opt);
		} else {
			pthread_t *tid;
			pthread_attr_t attr;
			thread_aux_t *data;
			...
			while(j < opt->n_threads) {
				IGP_id_thread_new=IGP_Get(0);
	
				if (IGP_id_thread_new>=0){
//				        fprintf(stderr, "[bwa_aln_core] id_thread_new [%d] \n", IGP_id_thread_new);				
					data[j].tid = j; data[j].bwt = bwt;
					data[j].n_seqs = n_seqs; data[j].seqs = seqs; data[j].opt = opt;
					data[j].IGP_id_thread = IGP_id_thread_new; // Necesario para IGP

					rc = pthread_create(&tid[j], &attr, worker, data + j);
					if (rc){
			...
			fprintf(stderr, "[bwa_aln_core] active_threads[%d]\n", active_threads);
	
			for (i = 0; i < opt->n_threads; ++i){
				rj = pthread_join(tid[i], NULL);
				if (rj) {
					fprintf(stderr, "ERROR; return code from pthread_join() is %d for %d thread.\n", rj, j);
					exit(1);
				}
			}
...
int bwa_aln(int argc, char *argv[])
{
	...
	pthread_create(&thconsumo, NULL, *consumoTotal, (void *) consumo);*/
